<template>
  <div class="recorder-container d-flex align-items-center">
    <div class="full d-flex">
      <canvas class="padding-a-2" id="voice-client" width="100%" height="96"></canvas>
      <canvas class="padding-a-2" id="mfcc-client" width="224" height="224"></canvas>
  ...
  methods: {
    ...
    async analyed(features){
      if(this.recording){
        ...
        let h = features["rms"] * 800;
        this.voiceCtx.fillStyle = '#FFA500';
        this.voiceCtx.fillRect(this.prevX, (this.canvasSize[1] - h) / 2, x - this.prevX, h);
        this.prevX = x;
        if(x >= this.canvasSize[0]){
          this.endRecord();
        }
      }else{
    ...
    async initRecord(){
      this.voiceCanvas = document.getElementById("voice-client");
      this.voiceCtx = this.voiceCanvas.getContext("2d");
      this.voiceCanvas.style.width = "100%";
      this.voiceCanvas.style.height = "100%";
      this.voiceCanvas.style.flexGrow = "1";

      this.voiceCanvas.width = this.voiceCanvas.offsetWidth;
      this.voiceCanvas.height = this.voiceCanvas.offsetHeight;
      this.canvasSize = [this.voiceCanvas.width, this.voiceCanvas.height];
    ...
    startRecord(){
      ...
      this.prevX = 0;
      this.prevMX = 0;
      this.voiceCtx.clearRect(0, 0, this.canvasSize[0], this.canvasSize[1]);
